...
declare const $: any;
@Component({
  selector: 'app-plo-analysis',
  templateUrl: './plo-analysis.component.html',
  styleUrls: ['./plo-analysis.component.css'],
  providers: [PagerService, HighlightPipe, FilterPipe]
})
export class PloAnalysisComponent implements OnInit {
  Institutes: [] = [];
  Department: [] = [];
  Intake: [] = [];
  PLOs_Analysis: any[] = [];
  ...
  GetPLOsResultsForSelctedIntake() {
    ...
    if (this.IsGradeFExcluded) {
      this.IsGradeFExcludedConditional = true;
      this._ReportsService.Get_PLOs_Analysis_For_Selected_Intake(Number(IntakeID)).
        subscribe(
          response => {
            try {
              if (response != null) {
