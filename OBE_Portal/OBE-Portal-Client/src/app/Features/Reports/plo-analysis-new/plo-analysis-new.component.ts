...
declare const $: any;
@Component({
  selector: 'app-plo-analysis-new',
  templateUrl: './plo-analysis-new.component.html',
  styleUrls: ['./plo-analysis-new.component.css'],
  providers: [PagerService, HighlightPipe, FilterPipe]
})
export class PloAnalysisNewComponent implements OnInit {
  Institutes: [] = [];
  Department: [] = [];
  Intake: [] = [];
  PLOs_Analysis: any[] = [];
  PLOs_Analysis_Graph: any[] = [];

  Is_Permission_Institute: boolean = false;
  Is_Permission_Deaprtment: boolean = false;
  Is_Permission_Faculty: boolean = false;
  Mapped_CLO_And_Attainment: any[] = [];
  query: string = "";
  pager: any = {};
  page: any;
  pagesize: number = 50;
  ...
  Selected_PLO: string;
  Selected_StudentID: number;
  Selected_Student_Name: string;
  Selected_Student_Enrollment: string;
  Selected_PLO_Attainment: number;
  Selected_PLO_Mapped_CLOs_Count: number;
  Selected_PLO_Mapped_CLO_Passed: number;
  Selected_PLO_Mapped_Not_Considered_CLO: number;
  Selected_PLO_Mapped_CLO_Failed: number;
  Selected_PLO_And_Student_Details: any[] = [];
  AllIntakeStudents: any[] = [];
  Temp_Institute_ID: number;
  Temp_Deaprtment_ID: number;
  IsGradeFExcluded: boolean;
  IsGradeFExcludedConditional: boolean;
  IndirectAssessmentMarks: any;
  constructor(
  ...
  GetPLOsResultsForSelctedIntake() {
    ...
    this._ReportsService.GetAllIntakeStudents(Number(IntakeID)).
      subscribe(
        response => {
          try {
            if (response != null) {
              this.AllIntakeStudents = response;
    ...
    if (this.IsGradeFExcluded) {
      this.IsGradeFExcludedConditional = true;
      this._ReportsService.Get_PLOs_Analysis_For_Selected_Intake_Average_Attainment(Number(IntakeID)).
        subscribe(
          response => {
            try {
              if (response != null) {
                ...
                for (var i = 0; i < this.dataset.length; i++) {
                  this.dataset[i].PLO_One_Attainment = Number(Math.round((this.dataset[i].PLO_One_Attainment / 100)));
                  this.dataset[i].PLO_Two_Attainment = Number(Math.round((this.dataset[i].PLO_Two_Attainment / 100)));
                  this.dataset[i].PLO_Three_Attainment = Number(Math.round((this.dataset[i].PLO_Three_Attainment / 100)));
                  this.dataset[i].PLO_Four_Attainment = Number(Math.round((this.dataset[i].PLO_Four_Attainment / 100)));
                  this.dataset[i].PLO_Five_Attainment = Number(Math.round((this.dataset[i].PLO_Five_Attainment / 100)));
                  this.dataset[i].PLO_Six_Attainment = Number(Math.round((this.dataset[i].PLO_Six_Attainment / 100)));
                  this.dataset[i].PLO_Seven_Attainment = Number(Math.round((this.dataset[i].PLO_Seven_Attainment / 100)));
                  this.dataset[i].PLO_Eight_Attainment = Number(Math.round((this.dataset[i].PLO_Eight_Attainment / 100)));
                  this.dataset[i].PLO_Nine_Attainment = Number(Math.round((this.dataset[i].PLO_Nine_Attainment / 100)));
                  this.dataset[i].PLO_Ten_Attainment = Number(Math.round((this.dataset[i].PLO_Ten_Attainment / 100)));
                  this.dataset[i].PLO_Eleven_Attainment = Number(Math.round((this.dataset[i].PLO_Eleven_Attainment / 100)));
                  this.dataset[i].PLO_Twelth_Attainment = Number(Math.round((this.dataset[i].PLO_Twelth_Attainment / 100)));
                ...
                this.PLOs_Analysis_Graph.push(
                  ["PLO1", Number(Math.round((this.PLO1 / this.dataset.length)))],
                  ["PLO2", Number(Math.round((this.PLO2 / this.dataset.length)))],
                  ["PLO3", Number(Math.round((this.PLO3 / this.dataset.length)))],
                  ["PLO4", Number(Math.round((this.PLO4 / this.dataset.length)))],
                  ["PLO5", Number(Math.round((this.PLO5 / this.dataset.length)))],
                  ["PLO6", Number(Math.round((this.PLO6 / this.dataset.length)))],
                  ["PLO7", Number(Math.round((this.PLO7 / this.dataset.length)))],
                  ["PLO8", Number(Math.round((this.PLO8 / this.dataset.length)))],
                  ["PLO9", Number(Math.round((this.PLO9 / this.dataset.length)))],
                  ["PLO10", Number(Math.round((this.PLO10 / this.dataset.length)))],
                  ["PLO11", Number(Math.round((this.PLO11 / this.dataset.length)))],
                  ["PLO12", Number(Math.round((this.PLO12 / this.dataset.length)))],
      ...
      this.IsGradeFExcludedConditional = false;
      this._ReportsService.GetPLOsAnalysisForSelectedIntakeIncludedFGrade(Number(IntakeID)).
        subscribe(
          response => {
            try {
              if (response != null) {
  ...
  Get_PLO_Sub_Details(StudentID, name, enrollement, PLO_Attainment, Total_CLO_Count, Passed_CLO_Count, Failed_CLO_Count, PLOID) {
    this.IndirectAssessmentMarks = this.dataset.filter(x => x.StudentID === StudentID)[0];
    this.ngxService.start();
  ...
  ExcludeStudentFromAnalysis(val: number) {
    Swal.fire({
      title: 'Are you sure?',
  ...
  ExcludeCLOFromAnalysis(val: number) {
    Swal.fire({
  ...
  GradeFCheck(val: boolean) {
    this.IsGradeFExcluded = val;
  }
  ViewAllStudents() {
    $("#AllStudents").modal('show');
  }
}
