...
@Component({
  selector: 'app-plo-coverage-coursewise',
  templateUrl: './plo-coverage-coursewise.component.html',
  styleUrls: ['./plo-coverage-coursewise.component.css'],
  providers: [PagerService, HighlightPipe, FilterPipe]
})
export class PloCoverageCoursewiseComponent implements OnInit {
  Institutes: [] = [];
  Department: [] = [];
  Intake: [] = [];
  PLO_Coverage: any[] = [];
  PLOs_Coverage_Graph
  PLOs_Coverage_Details: any[] = [];
  ...
  GetPLOsCoverageResultsForSelctedIntake() {
    ...
    this.ngxService.start();
    this.pagesize = 50;
    this._ReportsService.Get_PLOs_Course_Wise_Details(Number(IntakeID)).
      subscribe(
        response => {
          try {
            if (response != null) {
              ...
              this.PLOs_Coverage_Graph.push(
                ["PLO1", Number(this.PLO1)],
                ["PLO2", Number(this.PLO2)],
                ["PLO3", Number(this.PLO3)],
                ["PLO4", Number(this.PLO4)],
                ["PLO5", Number(this.PLO5)],
                ["PLO6", Number(this.PLO6)],
                ["PLO7", Number(this.PLO7)],
                ["PLO8", Number(this.PLO8)],
                ["PLO9", Number(this.PLO9)],
                ["PLO10", Number(this.PLO10)],
                ["PLO11", Number(this.PLO11)],
                ["PLO12", Number(this.PLO12)],
              )
